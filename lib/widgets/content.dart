...
class MessageImagePreview extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final size = MessageMediaContainer.sizeOf(context);
    print("DEBUG MessageImagePreview: size=$size");
...
class MessageMediaContainer extends StatelessWidget {
  ...
  static const _aspectRatio = 2 / 3;  // height/width for 3:2 ratio

  static Size sizeForRealmMediaPreviewSize(RealmMediaPreviewSize previewSize) {
    final width = switch (previewSize) {
      RealmMediaPreviewSize.small => 100,
      RealmMediaPreviewSize.medium => 150,
      RealmMediaPreviewSize.large => 200,
      RealmMediaPreviewSize.unknown => 150,
    };
    return Size(width.toDouble(), width * _aspectRatio);
  }

  static Size sizeOf(BuildContext context) {
    final store = PerAccountStoreWidget.of(context);
    final previewSize = store.realmMediaPreviewSize;
    return sizeForRealmMediaPreviewSize(previewSize);
  }
...
class InlineImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final imageSize =
        ...
        : MessageMediaContainer.sizeOf(context);
